...
interface BracketViewProps {
  ...
  tournamentId?: string;
  liveUpdates?: boolean;
  /** Enable drag-and-drop reordering (demo only) */
  editable?: boolean;
...
// ── Player slot ───────────────────────────────────────────────────────────────
function PlayerSlot({
  id, players, isWinner, isBye, matchId, slot, dragging, onDragStart, onDrop, editable,
}: {
  ...
  return (
    <div
      ...
      className={[
        'px-3 flex items-center justify-between gap-1 border-b border-slate-100 overflow-hidden transition-colors select-none',
...
// ── Match card ────────────────────────────────────────────────────────────────
function MatchCard({
  match, players, topPx, editable, dragging, onDragStart, onDrop,
}: {
  match: Match;
  players: Player[];
  topPx: number;
  ...
  return (
    <div
      className={`absolute bracket-match ${statusClass} overflow-hidden`}
