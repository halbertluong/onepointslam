...
/**
 * The complete public registration flow: tournament details, the sign-up form,
 * Stripe payment for the entry fee, and the standalone donation path.
 *
 * Rendered both by the public /register route and by the director dashboard's
 * Registration tab, so a director takes payment through exactly the same code
 * a registrant does and the two can never drift apart.
 */
export default function RegistrationFlow({
  ...
  onRegistered?: () => void;
  /** Run as a tournament director: keeps the flow usable after the public link
   *  closes. Card payments are still charged and verified exactly as normal —
   *  the server checks this claim before honouring it. */
  ...
  useEffect(() => {
    ...
    init();
  }, [tournamentId, slug, directorEntry]);
  ...
  // ── Closed ──────────────────────────────────────────────────────────────────
  if (step === 'loading') {
    return <div className={`${SHELL} flex items-center justify-center text-slate-400`}>Loading…</div>;
  }
