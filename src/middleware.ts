...
export async function middleware(request: NextRequest) {
  ...
  // If Supabase is not configured (local dev without .env), treat everyone as unauthenticated
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
    const { pathname } = request.nextUrl;
    if (pathname.startsWith('/admin') || pathname.startsWith('/dashboard') || pathname.startsWith('/referee')) {
