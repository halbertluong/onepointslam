...
function PrizePlacesEditor({
  ...
  places: NonNullable<Tournament['settings']['prizePlaces']>;
  ticketPrice: number;
  onChange: (p: NonNullable<Tournament['settings']['prizePlaces']>) => void;
}) {
  function addPlace() {
    const next = places.length + 1;
    onChange([...places, { place: next, type: 'fixed' as const, value: 0 }]);
  }
  ...
  function update(i: number, field: 'type' | 'value', val: string) {
    ...
    ) as NonNullable<Tournament['settings']['prizePlaces']>);
  }
      ...
      {places.map((p, i) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <div className="flex items-center border border-slate-200 rounded-xl overflow-hidden flex-1">
            <span className="px-2.5 py-2 bg-slate-50 text-slate-400 text-sm border-r border-slate-200">{p.type === 'fixed' ? '$' : '%'}</span>
            <input
              type="number"
              min="0"
          ...
          {p.type === 'percentage' && ticketPrice > 0 && (
            <span className="text-xs text-slate-400 shrink-0 w-16 text-right">≈ ${((p.value / 100) * ticketPrice).toFixed(0)}/player</span>
          )}
          <button type="button" onClick={() => remove(i)} className="text-slate-300 hover:text-red-400 text-lg leading-none">×</button>
        </div>
      ))}
      {totalPct > 100 && <p className="text-xs text-red-500 font-semibold">⚠ Percentage total exceeds 100% ({totalPct}%)</p>}
    </div>
  );
}
